...
def merge_meshes(meshes):
    """
    Merge a list of meshio meshes (from parallel subdomains) into a single mesh.
    ...
    for mesh in meshes:
        ...
        # Shift cell indices by current point offset
        ...
        # Collect point data
        for var, data in mesh.point_data.items():
            point_data[var].append(data)

        # Collect cell data, keeping track of block order
        for block_idx, cell_block in enumerate(mesh.cells):
            typ = cell_block.type
            for var, data_list in mesh.cell_data.items():
                if block_idx < len(data_list) and data_list[block_idx] is not None:
                    cell_data[var][typ].append(data_list[block_idx])

    # Stack points
    points = np.vstack(all_points)

    # Build cells list preserving type order
    cells = []
    cell_block_map = {}
    for typ in sorted(cells_by_type.keys()):
        data = np.concatenate(cells_by_type[typ], axis=0)
        cells.append(meshio.CellBlock(typ, data))
        cell_block_map[typ] = len(cells) - 1

    # Concatenate point data
    point_data_merged = {}
    for var, arrays in point_data.items():
        point_data_merged[var] = np.concatenate(arrays, axis=0)

    # Reconstruct cell data with correct block ordering
    cell_data_merged = {}
    for var, type_dict in cell_data.items():
        per_block = [None] * len(cells)
        for typ, arrays in type_dict.items():
            block_idx = cell_block_map[typ]
            per_block[block_idx] = np.concatenate(arrays, axis=0)
        cell_data_merged[var] = per_block
...
def main():
    ...
    **Download** in ParaView-compatible formats for advanced post-processing.
    ...
    if exodus_files and (st.session_state.selected_dir != selected_dir or st.session_state.meshio_mesh is None):
        ...
        with st.spinner(f"Loading {len(exodus_files)} file(s) from {selected_subdir}..."):
            for file_path in sorted(exodus_files):
                mesh = load_exodus_data(file_path)
                if mesh:
                    meshes.append(mesh)
    ...
    with st.sidebar:
        if n_times > 1:
            time_step = st.slider("Timestep", 0, n_times-1, n_times-1, key="time_slider")
            if time_values is not None and len(time_values) == n_times:
                st.caption(f"Time = {time_values[time_step]:.4g}")
        else:
            time_step = 0
            st.info("Static mesh (no time steps)")
            ...
            for idx, (fmt_key, fmt_info) in enumerate(available_formats.items()):
                with cols[idx % len(cols)]:
                    ...
                    success, message, file_size = convert_mesh_format(
                        merged_mesh, export_path, fmt_key
                    )
